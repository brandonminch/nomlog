...
Run commands from the **monorepo root** (where `pnpm-workspace.yaml` lives) unless noted.
...
pnpm --filter nomlog-app run ios

# Option 2: Run with specific configuration
pnpm --filter nomlog-app run ios:dev

# Option 3: Run on iOS Simulator specifically
pnpm --filter nomlog-app run ios:simulator
```
...
pnpm --filter nomlog-app run build:dev

# Build development version for Android
pnpm --filter nomlog-app run build:dev:android

# Build preview version for iOS
pnpm --filter nomlog-app run build:preview

# Build preview version for Android
pnpm --filter nomlog-app run build:preview:android
```
...
- If you get port conflicts, try: `pnpm --filter nomlog-app run start -- --port 8083`
- For simulator issues, try: `pnpm --filter nomlog-app run ios:simulator`
...
4. Use `pnpm --filter nomlog-app run build:dev` to create a new development build when needed
