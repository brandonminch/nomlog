...
- **`pnpm` on CI**: `ci_post_clone.sh` walks up from `nomlog-app` to the directory containing **`pnpm-workspace.yaml`**, runs **`pnpm install --frozen-lockfile`** there, then **`pod install`**. It activates **pnpm 9.15.9** via Corepack (or `npm i -g` / `npx` fallback). Override with **`PNPM_VERSION`** if you bump the repo pin.
